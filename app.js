...
function loadState() {
  try {
    ...
    if (isNewDay) {
      ...
      state.streak     = s.streak     || 0;   // carry streak across days
      // Write a clean slate back so firebase-sync sees the new date immediately
      try {
        const clean = {
          ...s,
...
function onEnd() {
  ...
  playGong();
  if (isFocusMode()) {
    state.pomCount++;   // FIX: was never incremented — leaderboard always showed 0
    state.sessionNum++;
...
function addTopic() {
  ...
  if (!v) return;
  if (state.topics.length >= MAX_TOPICS) {
    addLog('⚠ Topic limit reached (max ' + MAX_TOPICS + ') — remove some first');
    return;
  }
...
function bindEvents() {
  ...
  $('logoutBtn')?.addEventListener('click', async () => {
    ...
    // FIX: reset _appInited so init() runs again on next login
    // Without this, signing back in shows a blank app shell
    _appInited = false;
    try {
...
function bindAuthEvents() {
  ...
  async function submitAuth() {
    ...
    // Wait for firebase-sync to expose auth functions (Promise queue, no polling)
    if (typeof window._authSignIn !== 'function') {
      const ready = await new Promise((resolve, reject) => {
        const t = setTimeout(() => reject(new Error('timeout')), 8000);
        const interval = setInterval(() => {
          if (typeof window._authSignIn === 'function') {
            clearInterval(interval); clearTimeout(t); resolve();
          }
        }, 50);
      }).catch(() => 'timeout');
      if (ready === 'timeout') {
