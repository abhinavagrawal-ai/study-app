...
const MAX_TOPICS = 50;               // prevent localStorage bloat + slow UI
...
window.showToast = showToast; // expose for firebase-sync.js

// ══════════════════════════════════════════
//  CUSTOM CONFIRM — replaces confirm()
// ══════════════════════════════════════════
function showConfirm(msg, onConfirm, { title = 'ARE YOU SURE?', danger = false } = {}) {
...
function finishOnboard() {
  ...
  const obs = $('onboardScreen');
  if (obs) { obs.classList.add('hidden'); obs.style.display = 'none'; }
  applyUserProfile();
  showWelcomeToast();
...
function editProfile() {
  showConfirm(`Change name or exam? Your study data stays intact.`, () => {
    try { localStorage.removeItem(PROFILE_KEY); } catch(e) {}
    const obs = $('onboardScreen');
    if (obs) { obs.classList.remove('hidden'); obs.style.display = ''; }
  }, { title: 'EDIT PROFILE?' });
}
...
function loadState() {
  try {
    ...
    if (isNewDay) {
      ...
      // Carry streak only if they studied yesterday (had at least 1 pomodoro)
      // If they missed a day, streak resets to 0
      const yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 1);
      const wasYesterday = new Date(s.savedAt).toDateString() === yesterday.toDateString();
      state.streak = (wasYesterday && (s.pomCount || 0) > 0) ? (s.streak || 0) : 0;
      try {
        const clean = {
          ...s,
          pomCount: 0, studySecs: 0, checkinCount: 0, breakCount: 0,
          topicsDoneCount: 0, sessionNum: 1,
          streak: state.streak,
          running: false, timerStartedAt: null,
          savedAt: Date.now(), savedDate: todayKey(),
        }
    ...
    if (state.running && state.timerStartedAt) {
      ...
      if (isFocusMode()) {
        state.studySecs = state._studySecsBase + (state.remainAtStart > 0 ? Math.min(elapsed, state.remainAtStart) : 0);
      }
    }
    return true;
...
function setMode(m) {
  ...
  if (m !== 'custom') {
    ...
    updateRing();
    {const _e_timerModeTag=$('timerModeTag');if(_e_timerModeTag)_e_timerModeTag.textContent=MODES[m].label}
    {const _e_ringTrack=$('ringTrack');if(_e_ringTrack)_e_ringTrack.style.stroke=MODES[m].color}
    {const _e_timerDigits=$('timerDigits');if(_e_timerDigits)_e_timerDigits.style.color='#fff'}
  }
  updateWidget();
  saveState();
}
...
function applyCustom() {
  ...
  state.remainSecs = state.totalSecs;
  updateRing();
  {const _e_timerModeTag=$('timerModeTag');if(_e_timerModeTag)_e_timerModeTag.textContent='CUSTOM ' + v + 'M'}
  updateWidget();
  saveState();
}
...
function start() {
  ...
  {const _e_playBtn=$('playBtn');if(_e_playBtn)_e_playBtn.textContent='⏸'}
...
function pause() {
  ...
  {const _e_ringTrack=$('ringTrack');if(_e_ringTrack)_e_ringTrack.style.stroke=MODES[state.mode].color}
  {const _e_timerDigits=$('timerDigits');if(_e_timerDigits)_e_timerDigits.style.color='#fff'}
...
function onEnd() {
  ...
  if (isFocusMode()) {
    state.pomCount++;
    state.sessionNum++;
    // Increment daily streak on the FIRST pomodoro of each day
    if (state.pomCount === 1) state.streak++;
    addLog('✅ Pomodoro #' + state.pomCount + ' complete!');
    {const _e_breakPomN=$('breakPomN');if(_e_breakPomN)_e_breakPomN.textContent=state.pomCount}
    showBreakPopup();
    showToast(`🍅 Pomodoro #${state.pomCount} done! Take a break.`, 'success', 4000);
    ...
    setMode('focus');
    showToast('💪 Break over — back to focus!', 'info', 3000);
...
function setWakeupInterval(mins) {
  ...
  {const _e_intStatus=$('intStatus');if(_e_intStatus)_e_intStatus.innerHTML=`Popup every <b>${state.wakeupMins} mins</b> during focus`}
...
function applyCustomInterval() {
  ...
  state.wakeupMins = v;
  {const _e_intStatus=$('intStatus');if(_e_intStatus)_e_intStatus.innerHTML=`Popup every <b>${v} mins</b> during focus`}
  $('intervalCustomRow')?.classList.remove('show');
...
function showCheckPopup() {
  ...
  {const _e_checkEmoji=$('checkEmoji');if(_e_checkEmoji)_e_checkEmoji.textContent=msg.e}
  {const _e_checkMsg=$('checkMsg');if(_e_checkMsg)_e_checkMsg.textContent=msg.m}
  {const _e_checkStreak=$('checkStreak');if(_e_checkStreak)_e_checkStreak.textContent=state.streak}
...
function confirmCheckin() {
  ...
  addLog('✓ Check-in #' + state.checkinCount + ' confirmed! 🔥');
  updateStats();
...
function addPresetTopic() {
  ...
  if (state.topics.length >= MAX_TOPICS) {
    addLog('⚠ Topic limit reached (max ' + MAX_TOPICS + ')');
    sel.value = ''; return;
  }
...
function addTopic() {
  ...
  if (!v) return;
  if (state.topics.find(t => t.name.toLowerCase() === v.toLowerCase())) {
    showToast('Topic already in list!', 'info');
    return;
  }
  if (state.topics.length >= MAX_TOPICS) {
    showToast(`Topic limit reached (max ${MAX_TOPICS}) — remove some first`, 'error');
    return;
  }
...
function setActiveTopic(i) {
  ...
  {const _e_topicChipText=$('topicChipText');if(_e_topicChipText)_e_topicChipText.textContent=state.topics[i].name}
...
function delTopic(i) {
  ...
  if (state.activeTopicIdx === i) {
    ...
    {const _e_topicChipText=$('topicChipText');if(_e_topicChipText)_e_topicChipText.textContent='— pick a topic from Topics tab —'}
...
function renderWeekHistory() {
  ...
  {const _e_wkPomodoros=$('wkPomodoros');if(_e_wkPomodoros)_e_wkPomodoros.textContent=totalPoms}
  {const _e_wkHours=$('wkHours');if(_e_wkHours)_e_wkHours.textContent=th > 0 ? `${th}h${tm}m` : `${tm}m`}
  {const _e_wkDays=$('wkDays');if(_e_wkDays)_e_wkDays.textContent=activeDays}
...
// ══════════════════════════════════════════
function resetAll() {
  showConfirm("Today's stats will be cleared. Topics are kept.", () => {
    pause();
    ...
    const lu1 = $('logUl');
    if (lu1) lu1.innerHTML = '<div class="log-row"><span class="log-t">--:--</span><span>Reset. Fresh start! 💪</span></div>';
    saveState();
    showToast('Stats reset! Fresh start 💪', 'success');
  }, { title: 'RESET TODAY?', danger: false });
}

function clearHistory() {
  showConfirm('ALL history will be permanently deleted. This cannot be undone.', () => {
    ...
    updateRing(); updateStats(); updateWidget();
    const lu2 = $('logUl');
    if (lu2) lu2.innerHTML = '<div class="log-row"><span class="log-t">--:--</span><span>All history cleared! 🗑</span></div>';
    saveState();
    showToast('All history cleared 🗑', 'info');
  }, { title: 'CLEAR ALL HISTORY?', danger: true });
}
...
function drawChart() {
  ...
  if (allVals.length > 0) {
    ...
    // Parse as local date (not UTC) to avoid off-by-one day in negative-offset timezones
    const [bY,bM,bD] = best.key.split('-').map(Number);
    const bd = new Date(bY, bM-1, bD);
    {const _e_mBestDay=$('mBestDay');if(_e_mBestDay)_e_mBestDay.textContent=DAY_NAMES[bd.getDay()]}
    {const _e_mAvgPom=$('mAvgPom');if(_e_mAvgPom)_e_mAvgPom.textContent=(allVals.reduce((s,d)=>s+d.val,0)/allVals.length).toFixed(1)}
  } else {
    {const _e_mBestDay=$('mBestDay');if(_e_mBestDay)_e_mBestDay.textContent='—'}; {const _e_mAvgPom=$('mAvgPom');if(_e_mAvgPom)_e_mAvgPom.textContent='0'}
  }
  let bestStreak=0, cur=0;
  days.forEach(d => { d.val>0?(cur++,bestStreak=Math.max(bestStreak,cur)):cur=0; });
  {const _e_mStreak=$('mStreak');if(_e_mStreak)_e_mStreak.textContent=bestStreak}
}

function drawHeatmap() {
  const history = loadHistory(), today = todayKey();
  {const _e_heatmapDayLabels=$('heatmapDayLabels');if(_e_heatmapDayLabels)_e_heatmapDayLabels.innerHTML=['S','M','T','W','T','F','S'].map(d=>`<div class="heatmap-day-label">${d}</div>`).join('')}
...
function exportCSV() {
  ...
  if (keys.length === 0) { showToast('No history to export yet!', 'info'); return; }
  const DAY = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
  const rows = [['Date','Day','Pomodoros','Study Time (mins)','Check-ins','Topics Done','Topics Total','Topics List']];
  keys.forEach(k => {
    const d = history[k];
    // Parse as local date to avoid off-by-one in negative-offset timezones
...
function toggleWidgetMin() {
  ...
  {const _e_fwMinBtn=$('fwMinBtn');if(_e_fwMinBtn)_e_fwMinBtn.textContent=state.widgetMinimized ? '+' : '–'}
  updateWidget();
}
...
function bindEvents() {
  ...
  $('lbSyncBtn')?.addEventListener('click', () => {
    if (typeof window._syncStudyDB === 'function') { try { window._syncStudyDB(); } catch(e) {} }
  });

  // Logout
  $('logoutBtn')?.addEventListener('click', async () => {
    showConfirm('Your data is saved in the cloud and will be waiting when you sign back in.', async () => {
      try { pause(); } catch(e) {}
      try {
  ...
  // Confirm modal — dismiss on outside click
...
function init() {
  ...
  {const _e_ringTrack=$('ringTrack');if(_e_ringTrack)_e_ringTrack.style.strokeDasharray=CIRC}
  {const _e_ringTrack=$('ringTrack');if(_e_ringTrack)_e_ringTrack.style.strokeDashoffset=0}
  ...
  {const _e_timerModeTag=$('timerModeTag');if(_e_timerModeTag)_e_timerModeTag.textContent=MODES[state.mode].label}
  {const _e_ringTrack=$('ringTrack');if(_e_ringTrack)_e_ringTrack.style.stroke=MODES[state.mode].color}
  ...
  // Restore goal UI
  const gs = $('dailyGoalSelect');
  if (gs) gs.value = String(state.dailyGoal);

  // Restore topics
  renderTopics();
  if (state.activeTopicIdx >= 0 && state.topics[state.activeTopicIdx]) {
    {const _e_topicChipText=$('topicChipText');if(_e_topicChipText)_e_topicChipText.textContent=state.topics[state.activeTopicIdx].name}
  }
  ...
  // Start clock (guard against duplicate intervals on re-login)
  if (!state._clockTimer)  state._clockTimer  = setInterval(tickClock, 1000);
  if (!state._widgetTimer) state._widgetTimer = setInterval(() => { try { updateWidget(); } catch(e) {} }, 500);
  tickClock();
...
function _applyAriaLabels() {
  const labels = {
    playBtn:        'Play or pause timer',
    resetTimerBtn:  'Reset timer',
    skipTimerBtn:   'Skip to next session',
    addTopicBtn:    'Add topic',
    checkCtaBtn:    'Confirm check-in',
    endBreakBtn:    'End break',
    widgetToggleBtn:'Show floating timer',
    fwMinBtn:       'Minimize widget',
    fwCloseBtn:     'Close widget',
    exportCsvBtn:   'Export history as CSV',
    resetAllBtn:    "Reset today's stats",
    clearHistoryBtn:'Clear all history',
    lbSyncBtn:      'Sync leaderboard',
    profileEditBtn: 'Edit profile',
    logoutBtn:      'Sign out',
    applyCustomBtn: 'Apply custom duration',
    applyIntervalBtn:'Apply custom interval',
    obStartBtn:     "Let's start",
  }
  ...
  document.querySelectorAll('.nav-btn').forEach(btn => {
    ...
    btn.setAttribute('aria-label', `Go to ${btn.dataset.tab} tab`);
  });
}
...
function bindAuthEvents() {
  ...
  function setAuthMode(m) {
    ...
    // Switch autocomplete so password managers behave correctly
  ...
  async function submitAuth() {
    ...
    // On signup: enforce a minimum strength (not just length)
    if (_authMode === 'signup') {
      const hasLetter = /[a-zA-Z]/.test(password);
      const hasDigit  = /[0-9]/.test(password);
      const allSame   = /^(.)\1+$/.test(password);          // e.g. 'aaaaaa'
      const common    = ['123456','password','111111','abc123','000000','123123'];
      if (common.includes(password.toLowerCase())) {
        errEl.textContent = '⚠ Password too common — add letters and numbers'; return;
      }
      if (allSame) {
        errEl.textContent = '⚠ Password too weak — mix letters and numbers'; return;
      }
      if (!hasLetter || !hasDigit) {
        errEl.textContent = '⚠ Password must have at least one letter and one number'; return;
      }
    }
    ...
    // Wait for firebase-sync to expose auth functions.
    // Uses _whenAuthReady promise queue (no polling) — resolves instantly if already ready,
    // or waits up to 8 seconds for Firebase SDK to initialise.
    if (typeof window._authSignIn !== 'function') {
      const ready = await new Promise(resolve => {
        const t = setTimeout(() => resolve('timeout'), 8000);
        if (typeof window._whenAuthReady === 'function') {
          window._whenAuthReady(() => { clearTimeout(t); resolve('ok'); });
        } else {
          // Ultra-rare fallback: _whenAuthReady itself not yet defined
          const poll = setInterval(() => {
            if (typeof window._authSignIn === 'function') {
              clearInterval(poll); clearTimeout(t); resolve('ok');
            }
          }, 50);
        }
      });
...
window._appAuthReady = function(user) {
  ...
  if (user) {
    ...
    if (onboard)  { onboard.classList.add('hidden'); onboard.style.display = 'none'; }
    // Reset auth form
