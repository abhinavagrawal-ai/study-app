...
function _tick() {
  ...
  if (state.remainSecs % 60 === 0 && state.mode === 'focus') playTick();
  if (state.mode === 'focus' && state.checkinStartedAt) {
...
function skipTimer() {
  if (state.running && state.timerStartedAt && state.mode === 'focus') {
    const elapsed = Math.floor((Date.now() - state.timerStartedAt) / 1000);
    state.studySecs = state._studySecsBase + Math.min(elapsed, state.remainAtStart);
  }
  pause();
...
function onEnd() {
  ...
  playGong();
  if (state.mode === 'focus') {
    state.pomCount++; state.sessionNum++;
...
document.addEventListener('visibilitychange', () => {
  ...
  if (state.remainSecs <= 0) {
    ...
    state.running = false;
    state.timerStartedAt = null;
    if (state.mode === 'focus' && elapsed >= state.remainAtStart) addLog('⚡ Session completed while away!');
    onEnd();
    return;
  }
  if (state.mode === 'focus' && state.checkinStartedAt) {
...
function updateRing() {
  ...
  if (state.remainSecs < 60 && state.mode === 'focus' && state.running) {
