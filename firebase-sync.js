...
// ── Auth helpers ──
const toEmail = username => username.toLowerCase().trim().replace(/[^a-z0-9_]/g,'_') + '@pom.study';

function getUserId() {
  // Prefer Firebase Auth UID (persists across devices)
  if (auth.currentUser) return auth.currentUser.uid;
  // Fallback: local UID (pre-auth users)
  try {
...
export async function authSignUp(username, password) {
  ...
  // Pre-save profile with username as name
  const existing = getLocal(PROFILE_KEY);
  if (!existing.name) {
    try { localStorage.setItem(PROFILE_KEY, JSON.stringify({ name: username, prep: '', dailyGoal: 8 })); } catch(e) {}
  }
  return cred.user;
}
...
// Load cloud profile into localStorage (for cross-device support)
async function loadCloudProfile() {
  const user = auth.currentUser;
  if (!user) return;
  const local = getLocal(PROFILE_KEY);
  // Only restore from cloud if local profile is missing or nameless
  ...
  try {
    ...
    if (snap.exists()) {
      ...
      // Use displayName from auth as fallback
      localStorage.setItem(PROFILE_KEY, JSON.stringify({
...
// ══ AUTO-SYNC every 30s ══
setInterval(() => { if (auth.currentUser) syncUser().catch(()=>{}); }, 30000);

// ══ AUTH STATE DRIVES EVERYTHING ══
onAuthStateChanged(auth, async user => {
  if (user) {
    // Restore cloud profile to localStorage if first login on this device
    await loadCloudProfile();
    // Set up presence & initial sync
    setupPresence();
    syncUser().catch(() => {});
  }
  // Tell app.js about auth state
